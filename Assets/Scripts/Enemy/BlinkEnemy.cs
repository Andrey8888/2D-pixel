...
public class BlinkEnemy : Actor
{
    ...
    public float AggressionCooldownTime = 10f;
    public float ExpectationCooldownTime = 2f;
    public float BlinkCooldownTime = 5f;
    ...
    void Normal_Update()
    {
        ...
        else //if (expectationCooldownTimer >= 0f)
        {
        ...
        if (BehaivourType == Behaivour.Aggression)  // спорно реализовано
        {
            ...
            if (moveX != 0 && CheckColInDir(new Vector2(moveX, 0), solid_layer)) // остановка от препятствия 
            {
                Speed.x = Speed.x = Calc.Approach(Speed.x, 0f, RunReduce * num * Time.deltaTime);
            }
            if (transform.position.x > EnemyDetection.PlayerPos.x) // разворот за игроком
            {
                if (turnCooldownTimer <= 0f)
                {
                    CharacterRotation();
